...
generous_n_dims <- round(2 * iD_n_dims, 0)
```
...
cds <- xfun::cache_rds({
  SeuratWrappers::as.cell_data_set(seu, assay = "RNA")
...
groupings <- 
  colnames(seu@meta.data)[!(colnames(seu@meta.data) %in% 
                              c("orig.ident", "species", 
                                paste0("bc", 1:3, rep(c("_well", "_wind"), 3))))]
```

```{r groupings_vs_reductions, class.source = 'fold-hide', fig.dim = get_fig_dims(n_plots = 3 * length(groupings), n_cols = 3), fig.cap = "Plot of all monocle3 dimensionality reductions, coloured by groupings"}
# plot all groupings vs all reductions
groupings_vs_reductions <- list()
purrr::walk(groupingd, function(grouping) {
