...
ref_annots <- curr_params$infercnv$reference_celltypes 
query_annots <- curr_params$infercnv$query_celltypes
...
for (sample in unique(seu@meta.data$sample)) {
  ...
  # do not proceed if no query cells or reference left
  enough_query <- any(grepl("malignant", infercnv_annots$infercnv_population)) 
  enough_ref <- length(filt_ref_annots) > 1
  
  # proceed
  if (enough_query == F) {
    message("<100 query cells. Cannot proceed.")
  } else if (enough_ref == F) {
    message("<100 reference cells. Cannot proceed") 
  } else {
...
```{r malig_classification, eval = proceed == T}
aneuploid_cells <-
  list.files(
    params$cache_dir, 
    recursive = T, 
