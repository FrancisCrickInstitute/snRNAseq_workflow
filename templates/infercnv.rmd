...
for (sample_i in unique(seu@meta.data$sample)) {
  ...
  if (!is.null(final_name)) {
    ...
    # save annots file
    infercnv_annots %>%
      readr::write_tsv(paste0(out_dir, "/infercnv_annots.tsv"), col_names = F)
    ...
    infercnv_obj <- 
      infercnv::CreateInfercnvObject(
        ...
        annotations_file = paste0(out_dir, "/infercnv_annots.tsv"),
    ...
    seu_i <- infercnv::add_to_seurat(seu_i, infercnv_output_path = out_dir)
    saveRDS(seu_i, paste0(out_dir, "/seu_infercnv.rds"))
  }
}
```
...
aneuploid_cells <-
  ...
  purrr::map(function(file) {
    readr::read_tsv(file) %>%
      dplyr::mutate(sample = file %>% gsub("/HMM.*", "", .) %>% gsub(".*infercnv_cache//", "", .))
  }) %>%
  dplyr::bind_rows() %>%
...
seu_malig <- seu[, seu@meta.data$label.infercnv == "Malignant_cells"]
saveRDS(seu_malig, "seu_infercnv_malignant.rds")
```
