...
cells <-
  ...
  dplyr::filter(label.reduced %in% c("Epithelial_cells", ref_celltypes), 
                (sample == params$sample | sample_site == "tumour")) %>%
...
if (nrow(query_cells) < 100) {
  ...
  if ("sample_celltype" %in% reference_cells$name) {
    ...
    message("must pool each reference celltype from the patient tumour sites together")
    ...
    message("must pool all reference celltypes from the patient tumour sites together")
    final_name <- "patient_pool"
  } else {
    message("insufficient reference cells across the patient tumour sites")
    final_name <- NULL
  }
}
```
