...
`r if ("malignant" %ni% SummarizedExperiment::colData(cds)$final_annotation) { "inferCNV requires a malignant cluster as a query for CNV analysis, but the final celltype annotations do not include any 'malignant' cells. Please run the package again with a 'malignant' group specified in order to run inferCNV." }`

```{r exit_if_no_malignant, eval = !("malignant" %in% SummarizedExperiment::colData(cds)$final_annotation)}
