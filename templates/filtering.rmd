...
seu_ls <-
  ...
  purrr::map(function(dir) {
    ...
    seu <- subset(x = seu, subset = sample == params$sample)
...
seu@misc$sample_metadata <-
  readr::read_tsv(params$sample_metadata_file, show_col_types = F) %>%
  dplyr::filter(sample == params$sample)
...
# save
saveRDS(seu, "seu_unfiltered.rds")
```
