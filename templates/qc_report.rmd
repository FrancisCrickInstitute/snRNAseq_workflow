...
package_dir <-
  if (Sys.info()["nodename"]=="Alexs-MacBook-Air-2.local") {
    "/Volumes/TracerX/working/VHL_GERMLINE/tidda/vhl/"
  } else {
    "/camp/project/tracerX/working/VHL_GERMLINE/tidda/vhl/"
  } 
library(devtools)
...
# load params file
# cluster: # params <- list(params_file = "test/params.json")
# local:   # params <- list(params_file = "test/local_params.json")
curr_params <- rjson::fromJSON(file = params$params_file) %>% type.convert(as.is = T)
...
knitr::opts_chunk$set(fig.align = "center",
                      cache.path = paste0(curr_params$output_dir, "/cache/"),
                      dpi = 300)
...
groupings <-
  ...
  { if (curr_params$remove_doublets) c(., "doublet") else . } 
...
knitr::kable(tibble::tibble(argument = names(curr_params), value = curr_params %>% purrr::map(paste, collapse = ", ") %>% unlist %>% substr(1, 100)))
```

# Output info {-}

Output files will be saved to **`r curr_params$output_dir`**.

Figures will be saved to **`r curr_params$output_dir`qc_report_files/figure-html/**.

Intermediate objects will be cached to **`r curr_params$output_dir`qc_report_cache/html/**.

# `Seurat` pre-processing

## Load data and apply gene-level filter

Load Parse Biosciences split-pipe pipeline output as a Seurat object with no cut-offs, remove any missing samples, add sample metadata, add summary statistics, and optionally subset samples. 

Then, filter nuclei per gene, based on the `min_n_nuclei_per_gene` argument. Genes that are present in very few nuclei of the dataset (e.g., <3) are uninformative and unlikely to help in differentiating between groups of nuclei. In general, most genes removed by this filtering step will be those not detected in any nuclei, which will help to trim the size of the object. Additionally, genes are removed that have entries in all nuclei but whose entries are all zero. 

The minimum number of nuclei per gene is set to **`r curr_params$min_n_feature_RNA`**.

``` {r load_data, warning = F}
seu <- xfun::cache_rds({
  
  # read in DGE mtx
  dge_mat <- Seurat::ReadParseBio(curr_params$data_dir)
  
  # read in cell metadata
  cell_metadata <-
    curr_params$data_dir %>%
  ...
  seu <-
    ...
    Seurat::CreateSeuratObject(
      ...
      min_genes = curr_params$min_n_feature_RNA,
      min_cells = curr_params$min_n_nuclei_per_gene
    )
  
  # conditionally add sample metadata
  if (curr_params$do_add_sample_metadata == T) {

    # read in sample metadata and add to misc slot
    seu@misc$sample_metadata <-
      readr::read_tsv(curr_params$sample_metadata_file, show_col_types = F)
  ...
  # conditionally add summary statistics
  if (curr_params$do_add_summary_stats == T) {
    
    seu@misc$summary_stats <-
      curr_params$summary_stats_file %>%
      readr::read_csv(show_col_types = FALSE) %>%
      dplyr::rename(statistic = 1) %>%
      tidyr::pivot_longer(cols = -c("statistic"), names_to = "sample") %>%
      dplyr::mutate(statistic = statistic %>% gsub(paste0(curr_params$genome, "\\_"), "", .)) %>%
      dplyr::filter(
...
```{r summary_statistics_plot, class.source = 'fold-hide', eval = curr_params$do_add_summary_stats == T, message = F, warning = F, fig.height = 15, fig.cap = "Plot of summary statistics by sample"}
p <-
  seu@misc$summary_stats %>%
...
# knitr::include_graphics(paste(parse_dir, experiment, genome, sublibrary, "all-well/figures/fig_tscp_cell_cutoff.png", sep = "/"))
```
...
The `remove_doublets` argument is set to **`r  curr_params$remove_doublets`**.
`r if (curr_params$remove_doublets == T) {"Doublets will be identified and removed." }`
`r if (curr_params$remove_doublets == F) {"A dummy column will be introduced, treating all nuclei as though they are singlets."}`

```{r identify_doublets, eval =  curr_params$remove_doublets == T, include =  curr_params$remove_doublets == T}
seu <- xfun::cache_rds({
  annotate_doublets(seu)
}, file = "seu_doublets.rds")
```

```{r ignore_doublets, eval =  curr_params$remove_doublets == F, include =  curr_params$remove_doublets == F}
seu$doublet <- 0
```

```{r detected_doublets, eval =  curr_params$remove_doublets == T, class.source = 'fold-hide', fig.cap = "Plot of detected doublets in each sample (using scDblFinder)"}
p_dat <- 
...
The `do_filtering` argument is set to **`r curr_params$do_filtering`**.
`r if (curr_params$do_filtering == T) {"Filters will be applied." }`
`r if (curr_params$do_filtering == F) {"Dummy filters will be set (min = -Inf, max = Inf) and all nuclei will be retained." }`

```{r get_filters}
seu <- get_filters(
  seu,
  curr_params$do_filtering,
  curr_params$remove_doublets,
  curr_params$min_n_count_RNA,
  curr_params$max_n_count_RNA,
  curr_params$min_n_feature_RNA,
  curr_params$max_n_feature_RNA,
  curr_params$max_percent_mito
)
```
...
### `r if (curr_params$do_filtering) {"Apply nucleus-level filters"}`

```{r filter_nuclei, eval = curr_params$do_filtering == T, include = curr_params$do_filtering == T}
...
```{r annotate_cell_cycle_phases, eval = curr_params$do_cell_cycle_scoring == T}
...
cds <- xfun::cache_rds({
  ...
  if (curr_params$do_cell_cycle_scoring == T) {
