...
get_singler_annot_label <- function(cds, annot, lvl) {
  ...
  dplyr::as_tibble(cds@colData) %>%
    ...
    {dplyr::left_join(dplyr::as_tibble(cds@colData) %>% dplyr::transmute(lvl = get(lvl)), multiple = "all", ., by = "lvl")} %>%
    dplyr::pull(label)
}
...
get_singler_annot_reduced <-	function(x){
  ...
  replacements <-
    ...
    tibble::enframe(name = "singler_annot_reduced", value = "singler_annot_main") %>%
    tidyr::unnest(cols = "singler_annot_main")
...
cds <- xfun::cache_rds({
  ...
  cds@colData <- 
    ...
    cbind(
      cds_singler %>%
        dplyr::as_tibble() %>%
        dplyr::select(singler_annot_fine = pruned.labels) %>%
        dplyr::left_join(
          singler_refs@colData %>%
            tibble::as_tibble() %>%
            dplyr::select(singler_annot_main = label.main,
                          singler_annot_fine = label.fine) %>%
            dplyr::distinct()
        ) %>%
        get_singler_annot_reduced()
    )
  # add main/fine labels for partitions/clusters
  for (lvl in lvls) {
    for (annot in c("singler_annot_fine", "singler_annot_reduced")) {
      cds@colData[, paste(lvl, annot, sep = "_")] <-
...
annotation_col <- 
  cds@colData[, c("cluster", "partition", "singler_annot_reduced")] %>% 
  as.data.frame()
...
cds_singler$main_labels <- SummarizedExperiment::colData(cds)$singler_annot_reduced
p <-
...
p <- list()
purrr::walk(lvls, function(lvl) {
  purrr::walk(c("singler_annot_fine", "singler_annot_reduced"), function(annot) {
...
p <-
  ...
  tibble::as_tibble() %>%
  ggplot2::ggplot(ggplot2::aes(x = cluster, fill = singler_annot_reduced)) +
  ggplot2::geom_bar() + 
  ditto_colours +
...
p <-
  dplyr::as_tibble(SummarizedExperiment::colData(cds)) %>%
  dplyr::group_by(sample, partition, cluster, singler_annot_reduced) %>%
  dplyr::count() %>%
  dplyr::ungroup() %>%
  ...
  dplyr::mutate(x = dplyr::case_when(x == 1 ~ "sample",
                                     x == 2 ~ "partition",
                                     x == 3 ~ "cluster",
                                     x == 4 ~ "singler_annot_reduced") %>%
                  factor(levels = c("sample", "partition", "cluster", 
                                    "singler_annot_reduced"))) %>%
  ...
  ggforce::geom_parallel_sets_labels(angle = 0, 
                                     nudge_x = c(
                                       ...
                                       rep(0.4, dplyr::n_distinct(SummarizedExperiment::colData(cds)$singler_annot_reduced))
                                     )) +
  ggplot2::theme_void() +
...
`r if (curr_params$annotate$markers_file == F) {"No markers_file given." }`

```{r exit_if_no_markers, eval = curr_params$annotate$markers_file == F, include = F}
knitr::knit_exit()
```
...
```{r modules_vs_clusters_heatmap, eval = multiple_clusters == T, echo = F, warning = F, message = F, fig.dim = c(10, 7), fig.cap = "Heatmap of all module scores for all monocle3 clusters"}
...
```{r modules_vs_partitions_heatmap, eval = multiple_partitions == T, echo = F, warning = F, message = F, fig.dim = c(10, 7), fig.cap = "Heatmap of all module scores for all monocle3 partitions"}
agg_mat <- 
  monocle3::aggregate_gene_expression(
    cds, 
    gene_group_df, 
...
# All cluster evidence

```{r summarise_annots, , class.source = 'fold-hide'}
...
readr::write_tsv(cell_groupings_summary, "cell_groupings_summary.tsv")
knitr::kable(cell_groupings_summary, digits = 3)
```

# Final celltype annotations

`r if (curr_params$annotate$annotations_file == F) {"No annotations_file given." }`

```{r exit_if_no_annots, eval = curr_params$annotate$annotations_file == F, include = F}
knitr::knit_exit()
```

```{r load_annots}
annots <- 
  curr_params$annotate$annotations_file %>%
  readr::read_tsv() %>%
