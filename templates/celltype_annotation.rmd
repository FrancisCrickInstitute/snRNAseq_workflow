...
knitr::opts_chunk$set(fig.align = "center",
                      dpi = 300, 
                      cache.path = paste0(curr_params$output$dir, "celltype_annotation/celltype_annotation_cache/"))
```
...
`r if (curr_params$celltype_annotation$markers_file == F) {"No markers_file given." }`

```{r exit_if_no_markers, eval = curr_params$celltype_annotation$markers_file == F}
knitr::knit_exit()
```

```{r cds_avail_markers, echo = F}
markers <-
  curr_params$celltype_annotation$markers_file %>%
  readr::read_tsv()
markers <- 
...
`r if (curr_params$celltype_annotation$celltype_annotations_file == F) {"No celltype_annotations_file given." }`

```{r exit_if_no_annots, eval = curr_params$celltype_annotation$celltype_annotations_file == F, include = F}
knitr::knit_exit()
```

```{r load_annots}
annots <- 
  curr_params$celltype_annotation$celltype_annotations_file %>%
  readr::read_tsv() %>%
