  ...
  if (nrow(markers) > 0 & "p_val_adj" %in% colnames(markers)) {
    markers <-
      ...
      dplyr::mutate(rank = rank(-abs(avg_log2FC)),
                    p_val_adj_no_0 = dplyr::case_when(p_val_adj == 0 ~ min(p_val_adj),
                                                     TRUE ~ p_val_adj))
  } else {
...
```{r abort_if_no_infercnv, eval = length(cnv_files) == 0, include = length(cnv_files) == 0}
# stop if none
knitr::knit_exit()
```
...
# exclude samples for which infercnv run was unsuccessful
